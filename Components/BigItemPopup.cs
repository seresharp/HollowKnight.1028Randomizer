...
namespace Randomizer.Components
{
    public class BigItemPopup : MonoBehaviour
    {
        ...
        static BigItemPopup()
        {
            Frames = new[]
            {
                Sprites.Get("Anim.BigItemFleur.0"),
                Sprites.Get("Anim.BigItemFleur.1"),
                Sprites.Get("Anim.BigItemFleur.2"),
                Sprites.Get("Anim.BigItemFleur.3"),
                Sprites.Get("Anim.BigItemFleur.4"),
                Sprites.Get("Anim.BigItemFleur.5"),
                Sprites.Get("Anim.BigItemFleur.6"),
                Sprites.Get("Anim.BigItemFleur.7"),
                Sprites.Get("Anim.BigItemFleur.8")
            };
        }

        public static GameObject Show(Serialized.PopupInfo info, PlayMakerFSM fsm = null, string eventName = null)
        {
            // Create base canvas
            ...
            popup._imagePrompt = Sprites.Get(info.Sprite);
            popup._takeText = Lang.Get(info.Take);
            popup._nameText = Lang.Get(info.Name);
            popup._buttonText = Lang.Get(info.Button);
            popup._descOneText = Lang.Get(info.Description1);
            popup._descTwoText = Lang.Get(info.Description2);
            popup._fsm = fsm;
        ...
        private IEnumerator ShowPopup()
        {
            ...
            GameObject egg = CanvasUtil.CreateImagePanel(gameObject, Sprites.Get("UI.egg"),
                new CanvasUtil.RectData(
                    new Vector2(Sprites.Get("UI.egg").texture.width / 1.65f,
                        Sprites.Get("UI.egg").texture.height / 1.65f), Vector2.zero,
